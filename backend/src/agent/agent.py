...
class ConversationAgent:
    ...
    async def stream_turn(self, text: str):
        ...
        try:
            ...
            async for tok in stream:                            # raw token deltas
                if not acc:
                    print(f"[latency] retrieve={t_ret:.0f}ms  llm_first_token="
                          f"{(_t.monotonic()-t0)*1000:.0f}ms (since turn start)", file=_s.stderr, flush=True)
                acc.append(tok)
                yield tok
    ...
    def _fold_summary(self) -> None:
        ...
        # said/recommended, so it "forgets what it was talking about".
