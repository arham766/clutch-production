...
from app import build_agent, build_identify
from contracts import CatalogEntry, SupportSession
...
async def test_build_agent_refuses_out_of_docs():
    ...
    assert ans is None or ans.is_refusal or "docs" in ans.text.lower()   # never invents
...
def test_make_scope_session_stamps_catalog():
    from app import make_scope_session
...
async def test_per_session_agents_are_isolated():
    ...
    scope = __import__("app").make_scope_session(catalog=CATALOG)
