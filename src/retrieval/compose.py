...
_STREAM_SYSTEM = (
    "You are Clutch's live voice support agent, speaking out loud. Ground every answer ONLY in the "
    "numbered SOURCES — never invent steps, specs, error codes, or part names.\n"
    "Each turn, do exactly ONE of these:\n"
    "1) If the SOURCES clearly answer the question, reply immediately in a natural, concise spoken "
    "style — 1-2 short sentences.\n"
    "2) If the question is vague or could mean several things, or the SOURCES only partly match, ask "
    "ONE short clarifying question instead of guessing (e.g. the exact symptom, the model, or which "
    "step they're on).\n"
    "3) Only when the SOURCES are clearly unrelated to the question, say you don't have that in the "
    "documentation and offer to connect them with a person.\n"
    "Never read citation numbers or markers aloud. Keep it short and conversational."
)

# Prefill cost (and thus TTFT) scales with input length, so the streaming composer is sent a trimmed
# context: only the strongest sources, each capped, and a short recent-history window. The agent's
# full recall (recall_n) is unchanged — this only bounds what reaches the model per turn.
_MAX_SOURCES = 3
_MAX_SRC_CHARS = 500
_MAX_HISTORY = 6
...
def _history_messages(history, summary: str) -> list[dict]:
    ...
    for h in (history or [])[-_MAX_HISTORY:]:            # cap recent turns sent to the model
...
async def gateway_compose_stream_raw(answer_query, chunks, *, client, model, history=None, summary=""):
    ...
    if not passing:
        ...
        return
    src_chunks = passing[:_MAX_SOURCES]                  # strongest sources only (prefill ∝ length)
    src = "\n".join(f"[{i + 1}] {c.text[:_MAX_SRC_CHARS]}" for i, c in enumerate(src_chunks))
